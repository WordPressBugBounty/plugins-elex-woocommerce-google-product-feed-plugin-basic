...
## Example
```PHP
// You can use the global elexGpfWPFluent() function
$user = elexGpfWPFluent()->table('users')->find(1);
...
// Or, simply use the global elexGpfWPFluent() function.
// It handles all the ncessary initial setup.
elexGpfWPFluent()->table('my_table')->where('name', '=', 'admin');
```
