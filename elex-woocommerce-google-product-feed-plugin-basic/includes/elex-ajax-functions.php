...
class Elex_Gpf_Ajax_Call {
	...
	public function elex_gpf_generate_feed() {
		...
		} elseif ( isset( $_POST['categories_choosen'] ) && ! empty( $_POST['categories_choosen'] ) && ! isset( $_POST['sel_google_cats'] ) ) {
	...
	public function elex_gpf_create_project( $currency_code, $project_title, $project_desc, $sale_country, $sel_google_cats, $product_ids, $prod_attr, $google_attr, $exclude_ids, $autoset_identifier_exists, $condition, $prepend_attr, $append_attr, $mime_type, $currency_conversion, $featured, $stock_check, $stock_quantity, $prod_sold_check, $sold_quantity, $prod_vendor, $stock_status, $last_chunk = false, $chunk = 0, $more_than_hundred = false, $is_edit_project = '', $file_to_edit_or_refresh = '' ) {
		...
		foreach ( $product_ids as $key => $val ) {
			...
			foreach ( $val as $ids ) {
						...
						for ( $i = 0; $i < $google_attr_count; $i++ ) {
							...
							if ( $value_from_condition ) {
								...
								if ( isset( $prepend_attr[ $i ] ) ) {
									...
									if ( '_weight' == $prepend_value[0] || '_length' == $prepend_value[0] || '_width' == $prepend_value[0] || '_height' == $prepend_value[0] ) {
										$prepend_text = preg_replace( '/[^\d\.]/', ' ', $prepend_text );
									}
				...
				wc_delete_product_transients( $ids );
			}
		}
	...
	public function elex_get_tax_and_ship_details_csv( $shipping_data, $tax_data, $product_detail_data, $subscription_cost_data, $csv, $seperated_value ) {
		...
		if ( ! empty( $tax_data ) ) {
			...
			if ( in_array( 'rate', $tax_keys ) ) {
				if ( isset( $tax_data['rate'] ) && ! empty( $tax_data['rate'] ) ) {
					$csv .= $tax_data['rate'] . ':';
				}
			} else {
				$csv .= ':';
				...
				if ( $value[0] && $value[2] ) {
					...
					if ( $attr_val ) {
						...
						if ( $check_cond && 'OR' == $condition[1] ) {
							$success = true;
							break;
						} elseif ( ! $check_cond && 'AND' == $condition[1] ) {
							$success = false;
							break;
						}
					}
	...
	public function elex_gpf_get_simple_product_attr_values( $prod_attr, $google_attr, $ids, $product, $product_details, $prefix, $google_cat, $product_attributes, $autoset_identifier_exists, $add_items, $currency_conversion, $sale_country ) {
		...
		if ( is_array( $this->setting_tab_fields ) && isset( $this->setting_tab_fields['wpml_language'] ) && has_filter( 'wpml_object_id' ) ) {
			...
			* Retrieve the translated ID of a product using the WPML plugin.
			* This code retrieves the translated ID of a product in the language specified in the 'wpml_language' setting tab field. It uses the 'wpml_object_id' filter provided by the WPML plugin to retrieve the translated ID.
			 *
			* @since 2.0
			*/
		...
		if ( 'rec' == $recom_values[0] ) {
		...
		} elseif ( ( strpos( $prod_attr, 'elex_text_val', 0 ) === 0 ) ) {
			$map_prod_attr_val = str_replace( 'elex_text_val', '', $prod_attr );
		} elseif ( isset( $product_attributes[ $prod_attr_key ] ) && 'meta' == $product_attributes[ $prod_attr_key ]['type'] ) {
			$product           = wc_get_product( $ids );
			...
			if ( '_weight' == $prod_attr ) {
				$map_prod_attr_val = $map_prod_attr_val . ' ' . $weight_unit;
			}
		...
		} elseif ( '_stock_status' == $prod_attr ) {
			$product           = wc_get_product( $ids );
		...
		} elseif ( 'post_title' == $prod_attr ) {
			if ( $wpml_product ) {
				$map_prod_attr_val = $wpml_product->get_name();
			} else {
				$map_prod_attr_val = $product->get_name();
			}
		} elseif ( 'post_content' == $prod_attr ) {
			if ( $wpml_product ) {
				$map_prod_attr_val = $wpml_product_details['description'] ? html_entity_decode( strip_tags( $wpml_product_details['description'] ) ) : '';

			} else {
				$map_prod_attr_val = $product_details['description'] ? html_entity_decode( strip_tags( $product_details['description'] ) ) : '';

			}       
		} elseif ( 'post_excerpt' == $prod_attr ) {
			if ( $wpml_product ) {
				$map_prod_attr_val = $wpml_product_details['short_description'] ? html_entity_decode( strip_tags( $wpml_product_details['short_description'] ) ) : '';

			} else {
				$map_prod_attr_val = $product_details['short_description'] ? html_entity_decode( strip_tags( $product_details['short_description'] ) ) : '';

			}
		} elseif ( 'price' == $prod_attr ) {
		...
		if ( '_regular_price' == $prod_attr ) {
			$map_prod_attr_val = $product->get_regular_price();
			if ( is_numeric( $currency_conversion ) && is_numeric( $map_prod_attr_val ) ) {
				$map_prod_attr_val *= $currency_conversion;
			}  
		}
		if ( '_sale_price' == $prod_attr ) {
			$map_prod_attr_val = $product->get_sale_price();
			if ( is_numeric( $currency_conversion ) && is_numeric( $map_prod_attr_val ) ) {
				$map_prod_attr_val *= $currency_conversion;
			}  
		}
