	...
	public function elex_gpf_update_cron_jobs() {
		...
		if ( ! empty( $feed_ids ) ) {
			foreach ( $feed_ids as $key => $id ) {
				...
				if ( ! empty( $manage_feed_data ) ) {
					$saved_data = array_merge( $saved_data, $manage_feed_data );
				}
				if ( is_array( $filtering_options ) ) {
				...
				if ( isset( $value['file'] ) && 'ready' == $value['pause_schedule'] ) {
					...
					$current_time = $current_time->format( 'G' );

					if ( 'weekly' == $value['refresh_schedule'] ) {
						$today = strtolower( current_time( 'l' ) );
						if ( is_array( $value['refresh_days'] ) && in_array( $today, $value['refresh_days'] ) && ( $current_time === $value['refresh_hour'] ) ) {
							$run = true;
							if ( isset( $value['modified_date'] ) && ! ( abs( strtotime( $value['modified_date'] ) - strtotime( current_time( 'd-m-Y' ) ) ) / ( 60 * 60 * 24 ) ) ) {
								   $run = false;
							}
						}                   
					} elseif ( 'monthly' == $value['refresh_schedule'] ) {
					...
					} elseif ( 'daily' == $value['refresh_schedule'] ) {

						if ( $current_time === $value['refresh_hour'] ) {
							$run = true;
							if ( isset( $value['modified_date'] ) && ! ( abs( strtotime( $value['modified_date'] ) - strtotime( current_time( 'd-m-Y' ) ) ) / ( 60 * 60 * 24 ) ) ) {
								$run = false;
							}
					...
					if ( $run ) { 
						...
						$currency_code       = isset( $value['currency_conversion_code'] ) ? $value['currency_conversion_code'] : '';
